...
importer.getData = async () => {
  ...
  data.stashes.map(s => { 
    stashes.push({ _id: s.id, public: s.public, account_name: s.account_name, last_character_name: s.last_character_name, stash: s.stash, stash_type: s.stash_type, league: s.league, items: s.items, items_length: s.items.length, update_ts: new Date() });
  });
